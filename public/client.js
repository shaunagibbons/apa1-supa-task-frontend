...
function displayFish() {
  ...
  filteredFishArr.forEach((fish) =>
    resultElement.appendChild(createFishCard(fish))
  );
}
...
function createFishCard(fish) {
  ...
  card.innerHTML = `
    ...
    <button type="button" class="toggleUpdateFish" data-fish-id="${
      fish.Id
    }">Update Fish</button>
    <form class="updateFishForm" data-fish-id="${
      fish.Id
    }" style="display: none;">
      ${createFormInputs(fish)}
      <button type="button" class="updateFishButton" data-fish-id="${
        fish.Id
      }">Submit</button>
    </form>
    <button type="button" class="deleteFishButton" data-fish-id="${
      fish.Id
    }">Delete Fish</button>
  `;
...
function createFormInputs(fish) {
  return ["Name", "Sell", "Shadow", "Where"]
    .map(
      (field) => `
    <div class="form-input">
      <label for="update${field}-${fish.Id}">${field}</label>
      <input type="${
        field === "Sell" ? "number" : "text"
      }" id="update${field}-${fish.Id}" class="update${field}" value="${
        fish[field]
      }" required>
    </div>
  `
...
async function addFish(event) {
  ...
  if (![Name, Sell, Shadow, Where].every(field => field)) {
    return alert("All fields are required!");
  }
  
  try {
    console.log("Adding new fish..."); // Debugging: Log add action
    const response = await fetch("http://localhost:3000/api/fish", {
      method: "POST",
    ...
    if (!response.ok) throw new Error((await response.json()).error || "Failed to add fish.");
    alert("Fish added successfully!");
    getFish(); // Reload the fish data
  } catch (error) {
    console.error("Error adding fish:", error); // Debugging: Log errors
...
async function updateFish(fishId) {
  ...
  if ([Name, Sell, Shadow, Where].some((field) => !field))
    return alert("All fields are required!");
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        (await response.json()).error || "Failed to update fish."
      );
...
function getFormValues(form) {
  return Object.fromEntries(
    ["Name", "Sell", "Shadow", "Where"].map((field) => [
      field,
      form.querySelector(`.update${field}`).value,
    ])
  );
}
...
function filterFish() {
  ...
  filteredFishArr = query
    ? fishArr.filter((fish) => fish.Name.toLowerCase().includes(query))
    : fishArr;
  displayFish();
}
